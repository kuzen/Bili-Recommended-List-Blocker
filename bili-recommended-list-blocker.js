    ...
    setBlockBtnEvent(recommendContainer) {
      recommendContainer.onclick = (ev) => {
        ...
        if (target.className.toLowerCase() === 'brlb-block-btn') {
          ...
          const cardInfo = cardView.getElementsByClassName('bili-video-card__info')[0];
          const uidStr = cardInfo.firstElementChild.href;
          if (uidStr.length > 0) {
            const uid = uidStr.substr(uidStr.lastIndexOf('/') + 1);
            console.log(`${uid} 已屏蔽`);
            if (this.blockList.add('uid', uid) === true) {
              this.blockCardView(cardView, uid, 233);
            }
    ...
    run(cardViewList) {
      for (const cardView of cardViewList) {
        if (this.isAd(cardView)) {
          // 推广链接
          this.blockCardView(cardView, 0, 233);
          this.addBlockBtn(cardView);
          this.setCardViewEvent(cardView);
        } else {
          // 普通视频
